...
> **STATUS: ✅ SHIPPED + STAGING-VALIDATED 2026-05-19.** All three PRs
> (`f3afe20`, `7a7a606`, `cf74958`) on `main`; smoke tests clean.
> See the "Shipped" section at the end of this doc for the final tally.
> The plan below documents the original intent; what actually shipped
> is recorded in commit history.
>
> Original planning header (kept for posterity): Author SDL + Claude,
> 2026-05-19. Picks up after Slice 5C
> (`64aa66f..643333d`, all 14 PRs validated on staging).
...
All three PRs on `main`, each validated on staging
(`spec-test-2.myshopify.com`). Commits:

| PR | SHA | Scope |
| --- | --- | --- |
| #1 Dashboard config deletion | `f3afe20` | Per-row `shortcutActions` Remove (gated to `productMissing`) + `Page.secondaryActions` "Delete orphaned (N)" + Modal confirm + Toast. Two new intents on `api.sdl3d.config.tsx` (`deleteConfig`, `deleteOrphanedConfigs` — latter batches Shopify `nodes(ids:)` resolve in groups of 100). No schema change. |
| #2 Bucket-folder reuse for 360° | `7a7a606` | `Sdl3dBucketFolderPicker` Modal in editor Media InspectorSection. Two new intents on `api.sdl3d.storage.tsx` (`listBucketFolders` paginates 10×1000 keys + groups by parent prefix + filters to ≥24 frame folders; `useBucketFolder` writes `imageSequenceJson` directly, skipping capture pipeline). New `ProductConfig.imageSequenceSource` column (migration `20260519160000_image_sequence_source`) defaults `"CAPTURE"`; folder-pick path sets `"BUCKET_FOLDER"`. |
| #3 Editor top-bar storage selector | `cf74958` | Loader returns `availableStorages`; Polaris `Select` + `Badge` (attention="override", undefined="default") in topbar, hidden when ≤1 row. Selected `storageId` passes to both `Sdl3dRawCaptureUploader` (stamps `Capture.storageId` via `signRawUpload`) and `Sdl3dBucketFolderPicker` (folder picker now respects the selector — covers the PR #2 follow-up the plan flagged). No schema change. Editor-state only — never persisted, doesn't touch `ShopStorage.isDefault`. |

**Validation footnotes:**
- PR #1 staging-validated after deploy gotcha (`docker build` was using stale local host source; corrected redeploy command now prefixes `git pull origin main`). See [[feedback-redeploy-command]] memory for the full chained command.
- PR #2 validated end-to-end: merchant pre-uploaded 36 frames to bucket → folder appeared in picker with preview thumbnail → "Use this folder" wrote `imageSequenceJson` + `frameCount=36` + `imageSequenceSource="BUCKET_FOLDER"` with no Capture row.
- PR #3 validated by adding a second `ShopStorage` row on staging; Select + override Badge confirmed visible only with ≥2 rows.

**Memory artifacts generated by this slice:**
- `feedback_redeploy_command.md` — captures the full chained redeploy command (build was a no-op without the `git pull` prefix AND without the container recreate). Verified twice on this slice.
- Updated `MEMORY.md` index entry for the redeploy command to flag the `git pull` prefix as load-bearing.

## Open follow-ups for whoever picks up next

Nothing blocking. Slice 7 candidates queued in the "Out of scope"
section above — top three by ROI:

1. **Editor-side "Delete this config" button** (needs navigation-after-delete handling).
2. **Per-product storage assignment in the dashboard** (column showing each product's last capture bucket + per-product override).
3. **Bucket folder re-validation** (optional "Validate frames" button on the folder picker; dimensions + ordering check).
